...
RefCANChannelImpl::RefCANChannelImpl(const ContextPtr& context,
    ...
    , lastCollectTime(0)
    , samplesGenerated(0)
    , rawFrameCapture(init.rawFrameCapture)
    {
    initProperties();
    propChangedInternal();
    createSignals();
...
void RefCANChannelImpl::initProperties()
{
    ...
    objPtr.getOnPropertyValueWrite("UpperLimit") += [this](PropertyObjectPtr& obj, PropertyValueEventArgsPtr& args) { propChanged(); };
    ...
    objPtr.getOnPropertyValueWrite("LowerLimit") += [this](PropertyObjectPtr& obj, PropertyValueEventArgsPtr& args) { propChanged(); };
}
