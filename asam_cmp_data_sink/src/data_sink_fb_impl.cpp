...
void DataSinkFbImpl::addCaptureModuleFromStatus(int index)
{
    ...
    const auto newFb = createWithImplementation<IFunctionBlock, CaptureModuleImpl>(context, functionBlocks, fbId, std::move(deviceStatus));
...
{
    const StringPtr fbId = fmt::format("capture_module_{}", captureModuleId);
    const auto newFb = createWithImplementation<IFunctionBlock, CaptureModuleImpl>(context, functionBlocks, fbId);
    functionBlocks.addItem(newFb);
    ++captureModuleId;
}
